...
/**
 * @desc  Kiểm tra user có đủ điều kiện đánh giá phim không
 * @route GET /api/reviews/eligibility/:movieId
 * @access Private
 */
export const checkEligibility = async (req, res, next) => {
  try {
    ...
    if (existingReview) {
      return res.json(new ApiResponse(200, { eligible: false, reason: 'Bạn đã đánh giá phim này rồi' }));
    }

    // Tìm tất cả suất chiếu của phim này
    const showtimeIds = await Showtime.find({ movie: movieId }).distinct('_id');

    // Kiểm tra có booking confirmed nào không
    ...
    if (!confirmedBooking) {
      return res.json(new ApiResponse(200, { eligible: false, reason: 'Bạn cần đặt vé thành công cho phim này mới được đánh giá' }));
    }

    return res.json(new ApiResponse(200, { eligible: true }));
  } catch (err) { next(err); }
};
...
export const createReview = async (req, res, next) => {
  try {
    ...
    // Kiểm tra user có booking confirmed cho phim này không
    const showtimeIds = await Showtime.find({ movie: movieId }).distinct('_id');
    const confirmedBooking = await Booking.findOne({
      user: userId,
      showtime: { $in: showtimeIds },
    ...
    if (!confirmedBooking) {
      throw new ApiError(403, 'Bạn cần đặt vé thành công cho phim này mới được đánh giá');
    }
